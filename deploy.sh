...
ssh -o "StrictHostKeyChecking no" $USER@$IP -p $PORT <<EOF
  ...
  DC_WEBSITE_DOMAIN=$DC_WEBSITE_DOMAIN DC_ASPNETCORE_ENVIRONMENT=$DC_ASPNETCORE_ENVIRONMENT DC_RABBITMQPASSWORD=$DC_RABBITMQPASSWORD DC_SQLPASSWORD=$DC_SQLPASSWORD DC_MONGOPASSWORD=$DC_MONGOPASSWORD DC_SMTPSERVER=$DC_SMTPSERVER DC_SMTPPORT=$DC_SMTPPORT DC_SMTPUSERNAME=$DC_SMTPUSERNAME DC_SMTPPASSWORD=$DC_SMTPPASSWORD docker-compose up -d
  docker ps
